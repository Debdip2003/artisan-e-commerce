import p_img1 from './p_img1.jpg'
import p_img2 from './p_img2.jpg'

import p_img3 from './p_img3.jpg'
import p_img4 from './p_img4.jpg'
import p_img5 from './p_img5.jpg'
import p_img6 from './p_img6.jpg'
import p_img7 from './p_img7.jpg'
import p_img8 from './p_img8.jpg'
import p_img9 from './p_img9.jpg'
import p_img10 from './p_img10.jpg'
import p_img11 from './p_img11.png'
import p_img12 from './p_img12.png'
import p_img13 from './p_img13.png'
import p_img14 from './p_img14.png'
import p_img15 from './p_img15.png'
import p_img16 from './p_img16.png'
import p_img17 from './p_img17.png'
import p_img18 from './p_img18.png'
import p_img19 from './p_img19.png'
import p_img20 from './p_img20.png'
import p_img21 from './p_img21.png'
import p_img22 from './p_img22.png'
import p_img23 from './p_img23.png'
import p_img24 from './p_img24.png'
import p_img25 from './p_img25.png'
import p_img26 from './p_img26.png'
import p_img27 from './p_img27.png'
import p_img28 from './p_img28.png'
import p_img29 from './p_img29.png'
import p_img30 from './p_img30.png'
import p_img31 from './p_img31.png'
import p_img32 from './p_img32.png'
import p_img33 from './p_img33.png'
import p_img34 from './p_img34.png'
import p_img35 from './p_img35.png'
import p_img36 from './p_img36.png'
import p_img37 from './p_img37.png'
import p_img38 from './p_img38.png'
import p_img39 from './p_img39.png'
import p_img40 from './p_img40.png'
import p_img41 from './p_img41.png'
import p_img42 from './p_img42.png'
import p_img43 from './p_img43.png'
import p_img44 from './p_img44.png'
import p_img45 from './p_img45.png'
import p_img46 from './p_img46.png'
import p_img47 from './p_img47.png'
import p_img48 from './p_img48.png'
import p_img49 from './p_img49.png'
import p_img50 from './p_img50.png'
import p_img51 from './p_img51.png'
import p_img52 from './p_img52.png'


import logo from './logo.png'
import hero_login from './hero_login.jpg'
import hero_img from './hero_img.jpg'
import hero_img1 from './hero_img1.jpeg'
import hero_img2 from './hero_img2.jpg'
import hero_img3 from './hero_img3.jpg'
import cart_icon from './cart_icon.png'
import bin_icon from './bin_icon.png'
import dropdown_icon from './dropdown_icon.png'
import exchange_icon from './exchange_icon.png'
import profile_icon from './profile_icon.png'
import quality_icon from './quality_icon.png'
import search_icon from './search_icon.png'
import star_dull_icon from './star_dull_icon.png'
import star_icon from './star_icon.png'
import support_img from './support_img.png'
import menu_icon from './menu_icon.png'
import about_img from './about_img.png'
import contact_img from './contact_img.jpg'
import razorpay_logo from './razorpay_logo.png'
import stripe_logo from './stripe_logo.png'
import cross_icon from './cross_icon.png'
import artisan1 from './artisan1.jpg'
import artisan2 from './artisan2.jpeg'
import artisan3 from './artisan3.jpg'
import artisan4 from './artisan4.png'
import artisan5 from './artisan5.png'
import artisan6 from './artisan6.jpg'
import artisan7 from './artisan7.jpg'
import artisan8 from './artisan8.jpg'
import artisan9 from './artisan9.jpeg'
import artisan10 from './artisan10.jpg'
import artisan11 from './artisan11.jpg'
import artisan12 from './artisan12.jpg'
import artisan13 from './artisan13.jpg'
import artisan14 from './artisan14.jpg'
import artisan15 from './artisan15.jpg'
import artisan16 from './artisan16.jpg'
import artisan17 from './artisan17.jpeg'
import artisan18 from './artisan18.jpeg'
import artisan19 from './artisan19.jpeg'
import artisan20 from './artisan20.jpeg'
import artisan21 from './artisan21.png'
import artisan22 from './artisan22.jpg'
import artisan23 from './artisan23.jpeg'
import artisan24 from './artisan24.jpeg'
import artisan_working from './artisan_working.jpg'
import user_icon from './user_icon.jpeg'
import ceo_img from './ceo_img.jpg'
import artisan_head_img from './artisan_head_img.jpg'
import cpo_img from './cpo_img.jpg'
import cto_img from './cto_img.png'
import featured_artisan from './featured_artisan.jpg'
import artisan_local from './artisan_local.webp'
import eco_impact from './eco_impact.webp'
import bamboo_workshop from './bamboo_workshop.png'
import block_print_workshop from './block_print_workshop.webp'
import weaving_workshop from './weaving_workshop.jpeg'
import pottery_workshop from './pottery_workshop.jpg'
import workshop_hero from './workshop_hero.webp'
import user1 from './user1.webp'
import user2 from './user2.webp'

import shawlImage from './shawlImage.webp';
import teaSetImage from './teaSetImage.jpeg';
import elephantImage from './elephantImage.webp';
import sareeImage from './sareeImage.jpg';
import thaliImage from './thaliImage.jpg';

import art_img1 from './art_img1.jpeg';
import art_img2 from './art_img2.webp';
import art_img3 from './art_img3.jpeg';
import art_img4 from './art_img4.webp';
import art_img5 from './art_img5.jpg';
import art_img6 from './art_img6.webp';
import art_img7 from './art_img7.jpg';
import art_img8 from './art_img8.jpg';
import art_img9 from './art_img9.jpg';
import art_img10 from './art_img10.jpg';
import art_img11 from './art_img11.webp';
import art_img12 from './art_img12.webp';
import art_img13 from './art_img13.webp';
import art_img14 from './art_img14.jpg';
import art_img15 from './art_img15.jpg';
import art_img16 from './art_img16.jpeg';
import art_img17 from './art_img17.jpg';
import art_img18 from './art_img18.webp';
import art_img19 from './art_img19.jpg';
import art_img20 from './art_img20.jpeg';

export const assets = {
    logo,
    hero_img,bamboo_workshop,weaving_workshop,block_print_workshop,pottery_workshop,workshop_hero,shawlImage,teaSetImage,elephantImage,sareeImage,thaliImage,
    hero_img1,
    hero_img2,
    hero_img3,
    cart_icon,
    dropdown_icon,
    exchange_icon,
    profile_icon,
    quality_icon,
    search_icon,
    star_dull_icon,
    star_icon,
    bin_icon,
    support_img,
    menu_icon,
    about_img,
    contact_img,
    razorpay_logo,
    stripe_logo,
    cross_icon,
    artisan1, artisan2, artisan3, artisan4, artisan5,
  artisan6, artisan7, artisan8, artisan9, artisan10,
  artisan11, artisan12, artisan13, artisan14, artisan15,
  artisan16, artisan17, artisan18, artisan19, artisan20,art_img1,art_img2,art_img3,art_img4,art_img5,
  art_img6,art_img7,art_img8,art_img9,art_img10,art_img11,
  art_img12,art_img13,art_img14,art_img15,art_img16,
    art_img17,art_img18,art_img19,art_img20,
  artisan21,artisan22,artisan23,artisan24,
  ceo_img,artisan_head_img,cpo_img,cto_img,user_icon,hero_login,artisan_working,featured_artisan,artisan_local,eco_impact,user1,user2
}

export const products = [
    {
        _id: "aaaaa",
        name: "Terracotta Pottery",
        description: "Believed to be one of the oldest of crafts, pottery has always been in vogue. Over the years, owing to popular demand, the art has expanded to include new designs under its umbrella. The clay is moulded into different shapes and sizes to make coffee mugs, wall panels, vases, animal motifs and statues of mythological figures.",
        price: 1000,
        image: [p_img1],
        category: "Pottery",
        subCategory: "Terracotta",
        sizes: ["5L", "7L", "8L"],
        date: 1716634345448,
        bestseller: true,
        region: "South",
        culture: "Kannadiga"
    },
    {
        _id: "aaaab",
        name: "Banjara Handicrafts",
        description: "The semi-nomadic tribes of Banjara are regarded as the patrons of this craft. The women make vivid patch work quilts abound in geometric patterns. About three to six layers of reused fabric are held together by running stitches. Silver, brass, gold, ivory, animal bone, mirrors and even plastic are transformed as embellishments on these garments. They are mainly found in the districts of Bellary, Bijapur, Raichur, Gulbarga and Belgaum.",
        price: 1200,
        image: [p_img2],
        category: "Handmade Clothing",
        subCategory: "Handwoven",
        sizes: ["15L", "20L", "30L"],
        date: 1716621345448,
        bestseller: true,
        region: "South",
        culture: "Kannadiga"
    },
    {
        _id: "aaaac",
        name: "Kinhal Wood Carving",
        description: "Kinhal wood carving is the lacquerware woodcraft that takes its name from a small town in Koppal district. The craft was highly patronized by the Vijayanagara kings. Using a paste of tamarind seeds and pebbles to stick carved parts of wood, the Chitragars or artists create exquisite figurines and idols. A mixture of pebble powder and liquid gum is then used to sculpt ornaments and jewellery on these figures. The breathtaking designs of the Kinhal art can be easily differentiated from other forms of art by its vibrant colours and intricate designs. Kinhal toys have been given the Geographical Indication (GI) tag by the World Trade Organisation.",
        price: 820,
        image: [p_img3],
        category: "Home Decor",
        subCategory: "Handwoven",
        sizes: ["2.7 inch", "3 inch", "4 inch"],
        date: 1716234545448,
        bestseller: true,
        region: "South",
        culture: "Kannadiga"
    },
    {
        _id: "aaaad",
        name: "Channapatna Lacquerware",
        description: "The toys, which are lightweight yet tough, were originally made exclusively from ivory wood and coated with lacquer made from vegetable dyes. Traditionally, the colours used are a mix of vegetable dyes and food-grade pigments. The natural dyes include turmeric for yellow, indigo powder for blue, vermillion or kumkum powder for orange and red, and katha (acacia tree extract) for brown. The colours used for these toys specially for infants/ children are non-toxic and thus safe. For the 2015 Republic Day parade, the Karnataka tableau featured Channapatna handicrafts and won the third prize.",
        price: 710,
        image: [p_img4],
     
        subCategory: "Topwear",
     
        date: 1716621345448,
        bestseller: true,
        region: "South",
        culture: "Kannadiga"
    },
    {
        _id: "aaaae",
        name: "Gokak Toys",
        description: "In Gokak, local artisans create bright and colorful wooden toys mainly in the form of fruits, vegetables, animals and birds. This craft is believed to be at least 200 years old. Here, the toys are made using a specialised soft variety of wood, usually hariwala, polki, and hale wood.",
        price: 830,
        image: [p_img5],
        category: "Home Decor",
        subCategory: "Woodwork",
        sizes: ["2.7 inch", "3 inch", "4 inch"],
        date: 1716622345448,
        bestseller: true,
        region: "South",
        culture: "Kannadiga"
    },
    {
        _id: "aaaaf",
        name: "Rosewood Carving",
        description: "The ancient craft of rosewood carving is known for its elaborate rosewood inlay work. The art has been practiced in Mysore for centuries and is said to have enjoyed the patronage of the Mysore Maharajas. Artisans meticulously even out the wood and prepare it for etching the designs. The heavy timber that rosewood is, it's used extensively in furniture, cabinetry and architectural woodwork as well.",
        price: 940,
        image: [p_img6],
        category: "Home Decor",
        subCategory: "Woodwork",
        sizes: ["2.7 inch", "3 inch", "4 inch"],
        date: 1716623423448,
        bestseller: true,
        region: "South",
        culture: "Kannadiga"
    },
    {
        _id: "aaaag",
        name: "The Dual Tone Devi danglers",
        description: "A pair of dual tone ( silver and golden) german silver Devi durga face earrings. They are Stud danglers. A must have for the coming festive season.",
        price: 990,
        image: [p_img7],
        category: "Jewelry",
        subCategory: "Metalwork",
        sizes: ["S", "L", "XL"],
        date: 1716621542448,
        bestseller: false,
        region: "East",
        culture: "Bengali"
    },
    {
        _id: "aaaah",
        name: "Earrings Design 3(Combo)",
        description: "Fabric earrings offer a lightweight, eco-friendly, and stylish accessory for any occasion. Crafted from vibrant fabrics, these unique earrings add a touch of color and charm to your look while supporting sustainable fashion. Perfect for casual or festive wear!",
        price: 740,
        image: [p_img8],
        category: "Jewelry",
        subCategory: "Leather Craft",
        sizes: ["Pack of 1", "Pack of 2", "Pack of 3"],
        date: 1716622345448,
        bestseller: false,
        region: "West",
        culture: "Gujarati"
    },
    {
        _id: "aaaai",
        name: "Woven Zephyr",
        description: "Eazy Breezy White Dress Made From Organic Cotton And Marble Dyed In Indigo.",
        price: 1000,
        image: [p_img9],
        category: "Handmade Clothing",
        subCategory: "Embroidery",
        sizes: ["M", "L", "XL"],
        date: 1716621235448,
        bestseller: false,
        region: "North",
        culture: "Punjabi"
    },
    {
        _id: "aaaaj",
        name: "Men handwoven shirt",
        description: "Long Sleeve 100 % handspun and handwoven stylish and comfortable shirt made in Villages.",
        price: 1100,
        image: [p_img10],
        category: "Men",
        category: "Handmade Clothing",
        subCategory: "Handwoven",
        sizes: ["S", "L", "XL"],
        date: 1716622235448,
        bestseller: false,
        region: "South",
        culture: "Tamil"
    },
    {
        _id: "aaaak",
        name: "Handwoven Punjabi Phulkari Dupatta",
        description: "Vibrant handwoven dupatta with traditional Phulkari embroidery from Punjab, made with pure cotton threads.",
        price: 1500,
        image: [art_img1],
        category: "Women",
        subCategory: "Topwear",
        craftType: "Handwoven",
        sizes: ["One Size"],
        date: 1716623345448,
        bestseller: true,
        region: "North",
        culture: "Punjabi"
    },
    {
        _id: "aaaal",
        name: "Terracotta Elephant Sculpture",
        description: "Handcrafted terracotta elephant sculpture from West Bengal, made using traditional techniques passed down through generations.",
        price: 1800,
        image: [art_img2],
        category: "Home Decor",
        subCategory: "Showpiece",
        craftType: "Terracotta",
        sizes: ["Medium"],
        date: 1716624445448,
        bestseller: false,
        region: "East",
        culture: "Bengali"
    },
    {
        _id: "aaaam",
        name: "Bamboo Craft Basket Set",
        description: "Set of 3 eco-friendly bamboo baskets from Assam, handwoven by local artisans using sustainable bamboo.",
        price: 1200,
        image: [art_img3],
        category: "Home Decor",
        subCategory: "Storage",
        craftType: "Bamboo Craft",
        sizes: ["Set of 3"],
        date: 1716625545448,
        bestseller: true,
        region: "Northeast",
        culture: "Assamese"
    },
    {
        _id: "aaaan",
        name: "Block Printed Cotton Kurta",
        description: "Traditional block printed cotton kurta from Rajasthan featuring authentic Dabu printing techniques.",
        price: 1200,
        image: [art_img4],
        category: "Men",
        subCategory: "Topwear",
        craftType: "Block Printing",
        sizes: ["S", "M", "L"],
        date: 1716626645448,
        bestseller: false,
        region: "West",
        culture: "Rajasthani"
    },
    {
        _id: "aaaao",
        name: "Madhubani Painting on Silk",
        description: "Authentic Madhubani painting on pure silk, depicting traditional motifs from Bihar.",
        price: 1500,
        image: [art_img5],
        category: "Art",
        subCategory: "Wall Hanging",
        craftType: "Madhubani Painting",
        sizes: ["Medium"],
        date: 1716627745448,
        bestseller: true,
        region: "East",
        culture: "Bihari"
    },
    {
        _id: "aaaap",
        name: "Kashmiri Pashmina Shawl",
        description: "Luxurious handwoven Pashmina shawl from Kashmir with intricate embroidery.",
        price: 1500,
        image: [art_img6],
        category: "Women",
        subCategory: "Winterwear",
        craftType: "Loom Weaving",
        sizes: ["One Size"],
        date: 1716628845448,
        bestseller: false,
        region: "North",
        culture: "Kashmiri"
    },
    {
        _id: "aaaaq",
        name: "Warli Art Tribal Mask",
        description: "Hand-painted Warli art tribal mask made by Maharashtra's indigenous artists using natural colors.",
        price: 1500,
        image: [art_img7],
        category: "Home Decor",
        subCategory: "Wall Art",
        craftType: "Warli Art",
        sizes: ["Medium"],
        date: 1716629945448,
        bestseller: false,
        region: "West",
        culture: "Marathi"
    },
    {
        _id: "aaaar",
        name: "Kanjivaram Silk Saree",
        description: "Authentic Kanjivaram silk saree from Tamil Nadu with pure zari work, handwoven by master weavers.",
        price: 1200,
        image: [art_img8],
        category: "Women",
        subCategory: "Ethnicwear",
        craftType: "Loom Weaving",
        sizes: ["Free Size"],
        date: 1716631045448,
        bestseller: true,
        region: "South",
        culture: "Tamil"
    },
    {
        _id: "aaaas",
        name: "Metalwork Brass Ganesha Idol",
        description: "Handcrafted brass Ganesha idol from Gujarat, made using traditional metal casting techniques.",
        price: 1800,
        image: [art_img9],
        category: "Home Decor",
        subCategory: "Showpiece",
        craftType: "Metalwork",
        sizes: ["Small"],
        date: 1716632145448,
        bestseller: false,
        region: "West",
        culture: "Gujarati"
    },
    {
        _id: "aaaat",
        name: "Leather Craft Kolhapuri Chappals",
        description: "Genuine Kolhapuri leather chappals handcrafted by artisans from Maharashtra.",
        price: 1200,
        image: [art_img10],
        category: "Men",
        subCategory: "Footwear",
        craftType: "Leather Craft",
        sizes: ["7", "8", "9", "10"],
        date: 1716633245448,
        bestseller: true,
        region: "West",
        culture: "Marathi"
    },
    {
        _id: "aaaau",
        name: "Beadwork Tribal Necklace",
        description: "Colorful tribal necklace from Odisha made with traditional beadwork techniques.",
        price: 1800,
        image: [art_img11],
        category: "Women",
        subCategory: "Jewelry",
        craftType: "Beadwork",
        sizes: ["Adjustable"],
        date: 1716634345448,
        bestseller: false,
        region: "East",
        culture: "Odia"
    },
    {
        _id: "aaaav",
        name: "Wooden Carved Elephant Set",
        description: "Set of 3 hand-carved wooden elephants from Karnataka, made from sustainable rosewood.",
        price: 1500,
        image: [art_img12],
        category: "Home Decor",
        subCategory: "Showpiece",
        craftType: "Woodwork",
        sizes: ["Set of 3"],
        date: 1716635445448,
        bestseller: false,
        region: "South",
        culture: "Kannadiga"
    },
    {
        _id: "aaaaw",
        name: "Kids Handwoven Cotton Kurta",
        description: "Traditional handwoven cotton kurta for kids from Telangana with natural dyes.",
        price: 1200,
        image: [art_img13],
        category: "Kids",
        subCategory: "Ethnicwear",
        craftType: "Handwoven",
        sizes: ["3-4Y", "5-6Y", "7-8Y"],
        date: 1716636545448,
        bestseller: true,
        region: "South",
        culture: "Telugu"
    },
    {
        _id: "aaaax",
        name: "Goan Seashell Wind Chime",
        description: "Beautiful wind chime made with authentic Goan seashells and brass bells.",
        price: 600,
        image: [art_img14],
        category: "Home Decor",
        subCategory: "Garden Decor",
        craftType: "Beadwork",
        sizes: ["Medium"],
        date: 1716637645448,
        bestseller: false,
        region: "West",
        culture: "Goan"
    },
    {
        _id: "aaaay",
        name: "Himachali Woolen Shawl",
        description: "Warm handwoven woolen shawl from Himachal Pradesh with traditional patterns.",
        price: 900,
        image: [art_img15],
        category: "Women",
        subCategory: "Winterwear",
        craftType: "Loom Weaving",
        sizes: ["One Size"],
        date: 1716638745448,
        bestseller: false,
        region: "North",
        culture: "Himachali"
    },
    {
        _id: "aaaaz",
        name: "Stone Carved Buddha Head",
        description: "Exquisite stone carved Buddha head from Odisha, made from locally sourced sandstone.",
        price: 1500,
        image: [art_img16],
        category: "Home Decor",
        subCategory: "Showpiece",
        craftType: "Stone Carving",
        sizes: ["Large"],
        date: 1716639845448,
        bestseller: true,
        region: "East",
        culture: "Odia"
    },
    {
        _id: "aaaba",
        name: "Embroidery Mirror Work Cushion",
        description: "Hand-embroidered cushion cover with mirror work from Gujarat.",
        price: 1500,
        image: [art_img17],
        category: "Home Decor",
        subCategory: "Cushions",
        craftType: "Embroidery",
        sizes: ["16x16 inches"],
        date: 1716640945448,
        bestseller: false,
        region: "West",
        culture: "Gujarati"
    },
    {
        _id: "aaabb",
        name: "Assamese Silk Mekhela Chador",
        description: "Traditional Assamese silk Mekhela Chador set with golden Muga silk.",
        price: 1800,
        image: [art_img18],
        category: "Women",
        subCategory: "Ethnicwear",
        craftType: "Loom Weaving",
        sizes: ["Free Size"],
        date: 1716642045448,
        bestseller: true,
        region: "Northeast",
        culture: "Assamese"
    },
    {
        _id: "aaabc",
        name: "Kerala Kasavu Mundu Set",
        description: "Authentic Kerala Kasavu cotton Mundu set with golden borders for men.",
        price: 1200,
        image: [art_img19],
        category: "Men",
        subCategory: "Ethnicwear",
        craftType: "Handwoven",
        sizes: ["M", "L", "XL"],
        date: 1716643145448,
        bestseller: false,
        region: "South",
        culture: "Malayali"
    },
    {
        _id: "aaabd",
        name: "Bidriware Metal Vase",
        description: "Traditional Bidriware metal vase from Karnataka with intricate silver inlay work.",
        price: 800,
        image: [art_img20],
        category: "Home Decor",
        subCategory: "Vase",
        craftType: "Metalwork",
        sizes: ["Medium"],
        date: 1716644245448,
        bestseller: false,
        region: "South",
        culture: "Kannadiga"
    }
]

